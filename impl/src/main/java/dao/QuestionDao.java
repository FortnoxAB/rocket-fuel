...
public interface QuestionDao {
    ...
    @Query("SELECT question.id, question.question, answer_accepted, question.title, question.bounty, question.votes, question.created_at, question,user_id, question.slack_id, \"user\".picture, \"user\".name as created_by FROM question  INNER JOIN \"user\" on \"user\".id = question.user_id WHERE question.user_id=:userId")
    Observable<Question> getQuestions(long userId, CollectionOptions collectionOptions);

    @Query("SELECT question.id, question.question, answer_accepted, question.title, question.bounty, question.votes, question.created_at, question,user_id, question.slack_id, \"user\".picture, \"user\".name as created_by " +
        "FROM question " +
    ...
    @Query("SELECT question.id, question, title, bounty, votes, answer_accepted, created_at, user_id, slack_id, \"user\".picture FROM question " +
        "INNER JOIN \"user\" on \"user\".id = question.user_id " +
        "WHERE user_id = :userId AND question.id=:questionId")
    Observable<Question> getQuestion(long userId, long questionId);

    @Query("SELECT question.id, question.question, question.title, question.bounty, question.votes, question.answer_accepted, question.created_at, question.slack_id, u.picture, u.name as created_by, u.id AS user_id FROM question LEFT JOIN \"user\" u on u.id = question.user_id WHERE question.id=:questionId")
